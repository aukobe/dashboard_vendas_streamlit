...
with st.expander('Colunas'):
    colunas = st.multiselect('Selecione as colunas', list(dados.columns), list(dados.columns)) #2º parametro: opcoes de colunas / 3º parametro: valores padroes selecionados no momento inicial

st.sidebar.title('Filtros')
with st.sidebar.expander('Nome do produto'):
    produtos = st.multiselect('Selecione os produtos', dados['Produto'].unique(), dados['Produto'].unique())
with st.sidebar.expander('Categoria do produto'):
    categoria = st.multiselect('Selecione as categorias', dados['Categoria'].unique(), dados['Categoria'].unique())
...
with st.sidebar.expander('Marca'):
    marcas = st.multiselect('Selecione as marcas', dados['Marca'].unique(), dados['Marca'].unique())
...
Categoria in @categoria and \
@preco[0] <= Valor <= @preco[1] and \
@frete[0] <= Frete <= @frete[1] and \
@data_compra[0] <= `Data da Compra` <= @data_compra[1] and \
Marca in @marcas and \
`Local da compra` in @local_compra and \
@avaliacao[0] <= `Avaliacao da compra` <= @avaliacao[1] and \
